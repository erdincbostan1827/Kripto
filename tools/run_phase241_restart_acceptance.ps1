...
param(
    ...
    [string]$OutputDirectory = ".phase241-restart",
...
function Write-PhaseResult {
    ...
    $payload = [ordered]@{
        schema_version = "1.0"
        classification = "PHASE241_RESTART_ACCEPTANCE_ORCHESTRATION"
        generated_at = [DateTimeOffset]::UtcNow.ToString("o")
        passed = $Passed
        repository = $Repository
        candidate_sha = $CandidateRef
        workflow = "Phase 241 Restart Acceptance"
        run_id = $RunId
        detail = $Detail
        blocker = $Blocker
    ...
    $resultPath = Join-Path $resolvedOutput "PHASE241_ORCHESTRATION_RESULT.json"
...
try {
    ...
    $expectedRunTitle = "Phase 241 Restart Acceptance $CandidateRef"

    $dispatchStarted = [DateTimeOffset]::UtcNow
    Invoke-Gh -Arguments @(
        "workflow", "run", "Phase 241 Restart Acceptance",
        "--repo", $Repository,
        "--ref", "main",
    ...
    do {
        ...
        if ($candidateRuns.Count -gt 0) {
            ...
            if ([string]::IsNullOrWhiteSpace($runId)) {
                throw "Discovered Phase 241 run does not expose databaseId."
            }
            break
        }
    ...
    if ([string]::IsNullOrWhiteSpace($runId)) {
        throw "Could not discover the dispatched Phase 241 Restart Acceptance run for exact candidate '$CandidateRef'."
    }
    ...
    $artifactName = "phase241-restart-acceptance-$CandidateRef"
    Invoke-Gh -Arguments @(
        "run", "download", $runId,
        "--repo", $Repository,
        "--name", $artifactName,
    ...
    $restartStatus = [string]$restart.groups.restart_drills
    $passed = (
        $workflowPassed -and
        ...
        $restart.challenge.verified -eq $true -and
        $runtime.challenge.trust_verified -eq $true -and
        $restart.challenge.trust_verified -eq $true -and
        $runtime.selected_all_pass -eq $true -and
        $restart.selected_all_pass -eq $true -and
        $runtimeStatus -eq "PASS" -and
        $restartStatus -eq "PASS"
    )

    if (-not $passed) {
        if ([string]::IsNullOrWhiteSpace($blocker)) {
            if ($runtime.challenge.verified -ne $true -or $restart.challenge.verified -ne $true) { $blocker = "RELEASE_CHALLENGE_NOT_VERIFIED" }
            elseif ($runtime.challenge.trust_verified -ne $true -or $restart.challenge.trust_verified -ne $true) { $blocker = "RELEASE_CHALLENGE_TRUST_NOT_VERIFIED" }
            elseif ($runtimeStatus -ne "PASS") { $blocker = "RUNTIME_GROUP_$runtimeStatus" }
            elseif ($restartStatus -ne "PASS") { $blocker = "RESTART_GROUP_$restartStatus" }
            elseif (-not $workflowPassed) { $blocker = "WORKFLOW_FAILED" }
            else { $blocker = "RESTART_ACCEPTANCE_NOT_ALL_PASS" }
        }
        Write-PhaseResult -Passed $false -RunId $runId -Detail "Exact-SHA runtime + restart acceptance did not PASS." -Blocker $blocker
        Write-Host "PHASE241_RESTART_ACCEPTANCE=FAIL"
    ...
    Write-Host "Restart group: $restartStatus"
    ...
    try {
        $existing = Join-Path ([System.IO.Path]::GetFullPath($OutputDirectory)) "PHASE241_ORCHESTRATION_RESULT.json"
        ...
        Write-Warning "Could not write Phase 241 result JSON: $($_.Exception.Message)"
    }
    if ($message -notlike "Phase 241 Restart Acceptance failed closed:*") {
        Write-Host "PHASE241_RESTART_ACCEPTANCE=FAIL"
    }
    throw
}
