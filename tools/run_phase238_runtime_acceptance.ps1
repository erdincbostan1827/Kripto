...
param(
    ...
    [string]$OutputDirectory = ".phase238-runtime",
...
function Write-PhaseResult {
    ...
    $payload = [ordered]@{
        schema_version = "1.0"
        classification = "PHASE238_RUNTIME_ACCEPTANCE_ORCHESTRATION"
        generated_at = [DateTimeOffset]::UtcNow.ToString("o")
        passed = $Passed
        repository = $Repository
        candidate_sha = $CandidateRef
        workflow = "Phase 238 Runtime Acceptance"
        run_id = $RunId
        detail = $Detail
        blocker = $Blocker
        production_ready = $false
        truth_policy = "Phase 238 PASS closes only the exact-SHA Docker/PostgreSQL/Redis runtime acceptance profile. It does not authorize LIVE trading or close restart/testnet/PITR/HA/WORM/signing gates."
    }
    $resolvedOutput = [System.IO.Path]::GetFullPath($OutputDirectory)
    New-Item -ItemType Directory -Force -Path $resolvedOutput | Out-Null
    $resultPath = Join-Path $resolvedOutput "PHASE238_ORCHESTRATION_RESULT.json"
...
try {
    ...
    $expectedRunTitle = "Phase 238 Runtime Acceptance $CandidateRef"

    $dispatchStarted = [DateTimeOffset]::UtcNow
    Invoke-Gh -Arguments @(
        "workflow", "run", "Phase 238 Runtime Acceptance",
        "--repo", $Repository,
        "--ref", "main",
    ...
    do {
        ...
        if ($candidateRuns.Count -gt 0) {
            ...
            if ([string]::IsNullOrWhiteSpace($runId)) {
                throw "Discovered Phase 238 run does not expose databaseId."
            }
            break
        }
    ...
    if ([string]::IsNullOrWhiteSpace($runId)) {
        throw "Could not discover the dispatched Phase 238 Runtime Acceptance run for exact candidate '$CandidateRef'."
    }
    ...
    $artifactName = "phase238-runtime-acceptance-$CandidateRef"
    Invoke-Gh -Arguments @(
        "run", "download", $runId,
        "--repo", $Repository,
        "--name", $artifactName,
    ...
    $workflowResultPath = Get-ChildItem -LiteralPath $resolvedOutput -Recurse -File -Filter "PHASE238_RUNTIME_RESULT.json" |
        Select-Object -First 1 -ExpandProperty FullName
    $manifestPath = Get-ChildItem -LiteralPath $resolvedOutput -Recurse -File -Filter "manifest_runtime.json" |
        Select-Object -First 1 -ExpandProperty FullName

    if (-not $workflowResultPath) {
        throw "Phase 238 artifact did not contain PHASE238_RUNTIME_RESULT.json."
    }
    if (-not $manifestPath) {
        throw "Phase 238 artifact did not contain manifest_runtime.json."
    }

    $workflowResult = Get-Content -LiteralPath $workflowResultPath -Raw | ConvertFrom-Json
    $manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json

    if ($workflowResult.classification -ne "PHASE238_RUNTIME_ACCEPTANCE_WORKFLOW_RESULT") {
        throw "Unexpected Phase 238 workflow result classification: $($workflowResult.classification)"
    }
    if ($manifest.classification -ne "EXTERNAL_ACCEPTANCE_EVIDENCE_BUNDLE") {
        throw "Unexpected runtime evidence classification: $($manifest.classification)"
    }
    if ($manifest.profile -ne "runtime") {
        throw "Unexpected external acceptance profile: $($manifest.profile)"
    }
    if ($manifest.environment.git_commit_sha -ne $CandidateRef) {
        throw "Runtime evidence is bound to a different git SHA: $($manifest.environment.git_commit_sha)"
    }

    if ($null -ne $workflowResult.PSObject.Properties["blocker"]) {
        $blocker = [string]$workflowResult.blocker
    }
    if ([string]::IsNullOrWhiteSpace($blocker) -and $null -ne $manifest.PSObject.Properties["blocker"]) {
        $blocker = [string]$manifest.blocker
    }

    $runtimeStatus = [string]$manifest.groups.runtime
    $passed = (
        $workflowPassed -and
    ...
    if (-not $passed) {
        if ([string]::IsNullOrWhiteSpace($blocker)) {
            ...
            else { $blocker = "RUNTIME_ACCEPTANCE_NOT_ALL_PASS" }
        }
        Write-PhaseResult -Passed $false -RunId $runId -Detail "Exact-SHA runtime acceptance did not PASS." -Blocker $blocker
        Write-Host "PHASE238_RUNTIME_ACCEPTANCE=FAIL"
        Write-Host "Candidate SHA: $CandidateRef"
        Write-Host "Workflow run id: $runId"
        Write-Host "Blocker: $blocker"
        throw "Phase 238 Runtime Acceptance failed closed: $blocker"
    }

    Write-PhaseResult -Passed $true -RunId $runId -Detail "Exact-SHA trusted Docker/PostgreSQL/Redis runtime acceptance verified." -Blocker ""
    Write-Host "PHASE238_RUNTIME_ACCEPTANCE=PASS"
    Write-Host "Candidate SHA: $CandidateRef"
    Write-Host "Workflow run id: $runId"
    ...
    try {
        $existing = Join-Path ([System.IO.Path]::GetFullPath($OutputDirectory)) "PHASE238_ORCHESTRATION_RESULT.json"
        ...
        Write-Warning "Could not write Phase 238 result JSON: $($_.Exception.Message)"
    }
    if ($message -notlike "Phase 238 Runtime Acceptance failed closed:*") {
        Write-Host "PHASE238_RUNTIME_ACCEPTANCE=FAIL"
    }
    throw
}
