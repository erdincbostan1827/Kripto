...
WORKFLOW = ROOT / ".github" / "workflows" / "phase242-pitr-acceptance.yml"
ORCHESTRATOR = ROOT / "tools" / "run_phase242_pitr_acceptance.ps1"
...
def test_phase242_workflow_is_exact_sha_runtime_plus_pitr_and_fail_closed() -> None:
    text = _text(WORKFLOW)

    assert "name: Phase 242 PITR Acceptance" in text
    assert "run-name: Phase 242 PITR Acceptance ${{ inputs.candidate_ref }}" in text
    ...
    assert "PITR_DRILL_COMMAND: ${{ secrets.PITR_DRILL_COMMAND }}" in text
    assert "PITR_EVIDENCE_JSON: ${{ secrets.PITR_EVIDENCE_JSON }}" in text
    assert "python scripts/generate_acceptance_challenge.py" in text
    assert "--profile runtime --confirm-real-target" in text
    assert "--profile pitr --confirm-real-target" in text
    assert text.index("--profile runtime --confirm-real-target") < text.index("--profile pitr --confirm-real-target")

    assert "manifest_runtime.json" in text
    assert "manifest_pitr.json" in text
    assert "PITR_TARGET_IDENTITY.json" in text
    assert "phase242-pitr-acceptance-${{ inputs.candidate_ref }}" in text
    assert "production_ready = $false" in text
    assert "It does not authorize LIVE trading" in text


def test_phase242_workflow_secret_scope_is_pitr_only() -> None:
    text = _text(WORKFLOW)
    for marker in (
        "secrets.ACCEPTANCE_CHALLENGE_VERIFY_COMMAND",
        "secrets.PITR_DRILL_COMMAND",
        "secrets.PITR_EVIDENCE_JSON",
    ):
...
def test_phase242_workflow_is_windows_native_and_isolated() -> None:
    text = _text(WORKFLOW)

    assert "shell: pwsh" in text
    assert "shell: bash" not in text
    assert "reports/external_acceptance/phase242_runtime.env" in text
    ...
    assert '"kripto_phase242_$shortSha"' in text
    assert "$env:COMPOSE_PROJECT_NAME = $projectName" in text
    assert "$env:APP_ENV_FILE = 'reports/external_acceptance/phase242_runtime.env'" in text
...
def test_phase242_orchestrator_correlates_exact_run_and_verifies_both_manifests() -> None:
    text = _text(ORCHESTRATOR)

    assert "CandidateRef -notmatch '^[0-9a-fA-F]{40}$'" in text
    assert '$expectedRunTitle = "Phase 242 PITR Acceptance $CandidateRef"' in text
    assert '"--json", "databaseId,createdAt,status,conclusion,displayTitle"' in text
    assert '($displayTitle -eq $expectedRunTitle)' in text
    assert '"workflow", "run", "Phase 242 PITR Acceptance"' in text
    assert '"-f", "candidate_ref=$CandidateRef"' in text
    assert '& gh run watch $runId --repo $Repository --exit-status' in text
    assert '$artifactName = "phase242-pitr-acceptance-$CandidateRef"' in text
    assert '"PHASE242_PITR_RESULT.json"' in text
    assert '"PITR_TARGET_IDENTITY.json"' in text
    assert '"manifest_runtime.json"' in text
    assert '"manifest_pitr.json"' in text
    assert '$identity.classification -ne "PHASE242_PITR_TARGET_IDENTITY_NOT_ACCEPTANCE_EVIDENCE"' in text
    ...
    assert '$pitr.profile -ne "pitr"' in text
    assert '$workflowResult.candidate_sha -ne $CandidateRef' in text
    assert '$runtime.challenge.verified -eq $true' in text
    assert '$pitr.challenge.verified -eq $true' in text
    assert '$runtime.challenge.trust_verified -eq $true' in text
    assert '$pitr.challenge.trust_verified -eq $true' in text
    assert '$runtimeStatus -eq "PASS"' in text
    assert '$pitrStatus -eq "PASS"' in text
    assert "PHASE242_PITR_ACCEPTANCE=PASS" in text
    assert "PHASE242_PITR_ACCEPTANCE=FAIL" in text
    assert "production_ready = $false" in text
