...
const nav:ReadonlyArray<readonly [string,string]>=[['/',t('nav.dashboard')],['/scanner',t('nav.scanner')],['/analysis',t('nav.analysis')],['/orders',t('nav.orders')],['/alerts',t('nav.alerts')],['/research',t('nav.research')],['/performance',t('nav.performance')],['/settings',t('nav.settings')]];
function Page({children}:{children:React.ReactNode}){return <ErrorBoundary><Suspense fallback={<Box sx={{p:4}}><CircularProgress aria-label="Yükleniyor"/></Box>}>{children}</Suspense></ErrorBoundary>}
export default function App(){
 ...
 const drawer=<List aria-label="Ana navigasyon">{nav.map(([to,label])=><ListItemButton key={to} href={to} onClick={()=>setMobileOpen(false)}><ListItemText primary={label}/></ListItemButton>)}</List>;
