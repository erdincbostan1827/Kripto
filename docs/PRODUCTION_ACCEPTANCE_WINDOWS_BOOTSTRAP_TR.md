...
Repository'yi klonladıktan sonra exact SHA'yı belirle ve foreground runner kur:
...
Foreground readiness PASS olduktan ve kullandığın Docker kurulumu servis hesabından erişilebilir olduğundan emin olduktan sonra kalıcı runner için temiz bir runner dizininde:
...
1. `Production Runner Readiness` — exact SHA — PASS
2. Protected environment variable/secrets gerçek değerlerle hazır
3. `Production Acceptance` — aynı immutable SHA — çalıştır
4. real-target preflight — PASS
5. restart/PITR/HA/WORM/TESTNET/provenance/campaign evidence — PASS
6. merged external evidence verification — PASS
7. final release gate — PASS
